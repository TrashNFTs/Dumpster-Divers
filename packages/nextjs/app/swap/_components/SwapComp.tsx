...
export function SwapComp() {
  ...
  const { data: jsonsOfOwnedDumpsterDiversOfWallet } = useReadTokenURIsUTF8(
    dumpsterDiverContract,
  ...
  const { data: vaultsOwnerOfs, refetch: getVaultOwnerOfs } = useReadOwnerOfsTrash(vaultContract?.address, minted, trashContract);
  ...
  const nfts = jsons.map((json, index) => (
    <NftCard key={"trash-" + index} buttonText={approves[index] === vaultContract?.address ? "Swap" : "Approve"} nft={json} onClaimed={
       ...
       ? 
       async ()=> { await mintDumpsterDiver({args: [BigInt(json.id)]}); refreshPageData() }
       :
